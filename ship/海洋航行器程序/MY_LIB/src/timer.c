...
//uint8_t  txbuffer[18]="zaizaiwoainia";	//串口发送数组
...
u8 odd;//number of odd in signal
u8 lr;//orientation
u8 lf,rf,lb,rb;
...
void TIM3_IRQHandler(void)  
{
  ...
  if(TIM_GetITStatus(TIM3,TIM_IT_Update)!=RESET)
	...
	/////////////////////////////////////////////////////////////////////////////
		if(++flag_control==1)   //检测时间40ms  电机舵机控制
		{
			flag_control=0;
		...
		if(++flag_caculate==15)   //检测时间300ms  同时喂狗
		{
			Feed_WatchDog();
			flag_caculate=0;
			odd=0;
			...
			for(k=0;k<16;k++)//判断是否触发  set bit to k
			{
					...
					if(CountTmp[k]>THRESHOLD_Middle)
							 ucRecSign |= 1<<k;
					else
							ucRecSign &= ~(1<<k);
			}
			
//			if(ucRecSign &= (1<<13)) //the last one 0 or 1
//				fil=0;
//			else
//				fil=1;
			
//			for(k=0;k<16;k++)//no 010
//			{
//					recsigntemp &= ~(1<<k);
//					if(ucRecSign &= (1<<k))
//					{
